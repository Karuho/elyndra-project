...
_KOTLIN_EXTENSIONS = {".kt"}
_KOTLIN_SCRIPT_EXTENSION = ".kts"
_PROJECT_MARKERS = (
    "pom.xml",
    "build.gradle",
    "build.gradle.kts",
...
class KotlinProjectInspectSkill:
    name = "kotlin.project_inspect"
    description = "Inspecciona un proyecto Kotlin/JVM sin ejecutar código ni wrappers."
    ...
    def approval_details(
        ...
        return _approval_details(
            ...
            decision.source,
            context.config.kotlin_tool_timeout_seconds,
            ["inspect-kotlin-project", str(root)],
...
class KotlinDescriptorValidateSkill:
    name = "kotlin.descriptor_validate"
    ...
    def approval_details(
        ...
        return _approval_details(
            ...
            decision.source,
            context.config.kotlin_tool_timeout_seconds,
            ["validate-kotlin-descriptors", str(root)],
    ...
    def execute(self, context: SkillContext, params: dict[str, Any]) -> SkillResult:
        ...
        lines = [
            "Descriptores Kotlin válidos." if ok else "Descriptores Kotlin con errores.",
            "",
            f"- Proyecto: `{root}`",
...
class KotlincCompileSkill:
    name = "kotlin.kotlinc_compile"
    description = "Compila Kotlin con kotlinc hacia una carpeta temporal, sin plugins libres."
    risk = RiskLevel.MEDIUM
    ...
    def approval_details(
        ...
        return _approval_details(
            ...
            ["kotlinc", "-d", "<temporal>", "@<archivos>"],
            "No se ejecutan clases ni se aceptan plugins o flags libres del compilador.",
        )
    ...
    def execute(self, context: SkillContext, params: dict[str, Any]) -> SkillResult:
        ...
        settings = _pipeline_settings(context, root, params)
        authorization = _authorize_project(context, root, params, settings=settings)
        files, truncated = _collect_kotlin_files(
            target,
            root=root,
            exclude_paths=settings["exclude_paths"],
            max_files=settings["max_kotlin_files"],
        )
        if truncated:
        ...
        if not files:
            return _skipped_result(
                self.name,
                root,
                "No se encontraron archivos Kotlin para compilar.",
                authorization,
            )
        tool = resolve_project_tool(root, "kotlinc")
        if tool.path is None:
            return _tool_unavailable(self.name, root, "kotlinc", authorization)
        with tempfile.TemporaryDirectory(prefix="elyndra-kotlinc-") as temp_dir:
            temp = Path(temp_dir)
            argfile = temp / "sources.txt"
            argfile.write_text(
                "\n".join(_kotlinc_argfile_value(path) for path in files),
                encoding="utf-8",
            )
            argv = [
                str(tool.path),
            ...
            target = settings.get("jvm_target")
            if target is not None:
                argv.extend(("-jvm-target", _format_jvm_target(target)))
        ...
        return _process_result(
            ...
            result,
            authorization,
            success="Compilación Kotlin correcta.",
            failure="kotlinc encontró problemas.",
            extra={"files_examined": len(files)},
        )


class KotlinBuildSkill:
    name = "kotlin.build_project"
...
class KotlinTestSkill:
    name = "kotlin.test_project"
...
class KotlinVerifyProjectSkill:
    name = "kotlin.verify_project"
    description = "Ejecuta la verificación Kotlin/JVM completa y guarda un historial comparable."
    risk = RiskLevel.MEDIUM
    ...
    def execute(self, context: SkillContext, params: dict[str, Any]) -> SkillResult:
        ...
        stage_specs = (
            ("inspect", True, KotlinProjectInspectSkill()),
            ("descriptor", settings["descriptor_enabled"], KotlinDescriptorValidateSkill()),
            ("kotlinc", settings["kotlinc_enabled"], KotlincCompileSkill()),
            ("build", settings["build_enabled"], KotlinBuildSkill()),
            ("tests", settings["tests_enabled"], KotlinTestSkill()),
        )
        ...
        heading = {
            "passed": "Verificación Kotlin correcta.",
            "partial": "Verificación Kotlin parcial.",
            "failed": "Verificación Kotlin fallida.",
        }[status]
        lines = [
            heading,
            "",
...
def _execute_build_stage(
    ...
    *,
    ...
    authorization = _authorize_project(context, root, params, settings=settings)
    build_tool = _selected_build_tool(root, settings)
    if build_tool == "kotlinc":
        if tests:
            return _skipped_result(
                skill_name,
                root,
                "No hay runner de tests configurado para un proyecto kotlinc directo.",
                authorization,
            )
        return KotlincCompileSkill().execute(context, params)
    ...
    return _process_result(
        ...
        result,
        authorization,
        success=f"{action} Kotlin finalizó correctamente.",
        failure=f"{action} Kotlin encontró problemas.",
...
def _build_argv(build_tool: str, *, tests: bool) -> list[str]:
    ...
    return ["kotlinc", "-d", "<temporal>", "@<archivos>"]
...
def _pipeline_settings(
    ...
    effective = context.kotlin_profiles.effective_settings(
        root,
        default_timeout_seconds=context.config.kotlin_tool_timeout_seconds,
        default_max_output_chars=context.config.kotlin_tool_max_output_chars,
    )
    ...
    return {
        ...
        "jvm_target": _jvm_target_value(
            params.get("jvm_target"),
            profile.get("jvm_target"),
        ),
...
def _disabled_stage_message(stage_name: str, settings: dict[str, Any]) -> str:
    if stage_name == "kotlinc" and settings.get("selected_build_tool") in {
        "maven",
        "gradle",
    }:
        return (
...
def _jvm_target_value(value: Any, current: Any) -> int | None:
...
def _format_jvm_target(value: int) -> str:
    return "1.8" if value == 8 else str(value)
...
def _collect_kotlin_files(
    ...
    *,
    ...
    if target.is_file():
        return ([target] if target.suffix.casefold() in _KOTLIN_EXTENSIONS else []), False
    excluded = {
    ...
    for current, directories, filenames in os.walk(target, followlinks=False):
        ...
        for filename in sorted(filenames):
            ...
            if candidate != root and root not in candidate.parents:
                continue
            if candidate.suffix.casefold() not in _KOTLIN_EXTENSIONS:
                continue
...
def _count_kotlin_scripts(root: Path, settings: dict[str, Any]) -> int:
    excluded = {
        (root / relative).resolve(strict=False)
        for relative in (*_DEFAULT_EXCLUDES, *settings["exclude_paths"])
    }
    count = 0
    for current, directories, filenames in os.walk(root, followlinks=False):
        ...
        for filename in filenames:
            candidate = (current_path / filename).resolve(strict=False)
            if candidate.suffix.casefold() != _KOTLIN_SCRIPT_EXTENSION:
                continue
            if candidate != root and root not in candidate.parents:
                continue
            if _is_excluded(candidate, excluded):
...
def _inspect_project(root: Path, settings: dict[str, Any]) -> dict[str, Any]:
    ...
    test_count = sum(
        ...
        or path.name.endswith(("Test.kt", "Tests.kt", "Spec.kt"))
    )
    wrappers = [
        name
    ...
    return {
        ...
        "kotlin_scripts": _count_kotlin_scripts(root, settings),
        ...
        "tools": {
            ...
            for name in ("kotlin", "kotlinc", "mvn", "gradle")
        },
    }
...
def _format_inventory(inventory: dict[str, Any], authorization: dict[str, Any]) -> str:
    ...
    return "\n".join(
        (
            "Inspección Kotlin completada sin ejecutar código.",
            "",
            f"- Proyecto: `{inventory['project_root']}`",
            f"- Archivos Kotlin: `{inventory['kotlin_files']}`",
            f"- Scripts Kotlin: `{inventory['kotlin_scripts']}`",
...
def _file_limit_result(
    ...
    return SkillResult(
        False,
        f"El proyecto supera el límite de {settings['max_kotlin_files']} archivos Kotlin.",
        {
            "engine": "local-skill",
            "generated": False,
            "skill": skill_name,
...
def _kotlinc_argfile_value(path: Path) -> str:
