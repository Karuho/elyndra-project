...
class SwiftProjectProfileRepository:
    """Persist safe defaults for controlled Swift verification."""
    ...
    def save(
        ...
        *,
        ...
        values = {
            ...
            "configuration": _choice(
                configuration,
                current.get("configuration", "debug"),
            ...
            "max_swift_files": _bounded_int(
                max_swift_files,
                current.get("max_swift_files", 3000),
                minimum=_MIN_FILES,
                maximum=_MAX_FILES,
                field="max_swift_files",
            ),
        ...
        with self.database.connect() as connection:
            connection.execute(
                """
                INSERT INTO swift_project_profiles(
                    project_root,
                    manifest_enabled,
                    syntax_enabled,
                    format_enabled,
                ...
                ON CONFLICT(project_root) DO UPDATE SET
                    ...
                    max_swift_files=excluded.max_swift_files,
        ...
        if profile is None:
            raise RuntimeError("No se pudo recuperar el perfil Swift guardado.")
        return profile
    ...
    def get(self, project_root: Path) -> dict[str, Any] | None:
        ...
        with self.database.connect() as connection:
            row = connection.execute(
                "SELECT * FROM swift_project_profiles WHERE project_root = ?",
                (str(root),),
            ).fetchone()
    ...
    def list_all(self) -> list[dict[str, Any]]:
        with self.database.connect() as connection:
            rows = connection.execute(
                "SELECT * FROM swift_project_profiles "
    ...
    def delete(self, project_root: Path) -> bool:
        ...
        with self.database.connect() as connection:
            cursor = connection.execute(
                "DELETE FROM swift_project_profiles WHERE project_root = ?",
                (str(root),),
            )
    ...
    def effective_settings(
        ...
        *,
        ...
        return {
            ...
            "max_swift_files": int(
                profile.get("max_swift_files", 3000) if profile else 3000
            ),
            "exclude_paths": list(
...
def _validate_project_root(project_root: Path) -> Path:
    ...
    if not root.is_dir():
        raise ValueError(f"El perfil Swift requiere una carpeta existente: {root}")
    return root
...
def _exclude_paths(
    ...
    for item in values:
        ...
        if relative.is_absolute() or ".." in relative.parts:
            raise ValueError("Las exclusiones Swift deben ser rutas relativas seguras.")
        resolved = (root / relative).resolve(strict=False)
        if resolved != root and root not in resolved.parents:
            raise ValueError("Una exclusión Swift sale del proyecto.")
